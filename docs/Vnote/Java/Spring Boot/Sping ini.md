...
### 1.2. Spring思想
#### 1.2.1. 从面向过程到面向对象
在软件业发展的初期，程序编写以算法为核心[^OO]。程序员会有意将数据当做问题空间的客体，程序代码则像锤子剪刀加之使用来处理这些数据。这是一种站在计算机角度来抽象问题和解决问题的思维方式，也即面向过程的编程思想。之后为了适应越来越繁杂的现实世界所提出的需求，诞生了面向对象的编程思想。面向对象鼓励程序站在现实世界与业务的角度去抽象和解决问题，如此数据和处理数据的逻辑与控制也被统一到了类与对象中。

[^OO]:深入理解Java虚拟机，周志明，2020年版

#### 1.2.2. 管理对象--IoC(Inversion of Control)
##### 1.2.2.1. IOC的诞生
...
#### 1.2.3. 全局对象封装--AOP（Aspect Oriented Programming）
##### 1.2.3.1. 传递代码块
...
##### 2.6.2.4. DTO层、Model层
与Entity层功能类似，但是其中模型类不与数据库中字段对应，处于数据转化的中间地位。
##### 2.6.2.5. Service层:业务层 设计业务逻辑
...
##### 2.6.2.6. Controllor层:控制层 控制业务逻辑
