...
## 1.3. Bug示例
### 1.3.1. 正则匹配
#### 1.3.1.1. 对于方法的了解---document.write()
**先自己跑一遍**
**确认代码无误**
但还是有问题,So?
    - 尝试1
...
把`.`换成`[\s\S]`会怎么样-->没有效果
    - Try5
那么可能是`+`的问题?你又翻看了定义,`匹配前面的子表达式一次或多次`.觉得不会是这个问题,但你还是改一下成`*`-->![](_v_images/20191215101234089_18272.png =94x),你发现多了个`,`号,这个是啥?-->**你猜**这个是document自己加的,其后面为空,代表0次出现.**但是因为和主问题无关,所以先不管了**
**那么似乎已经尝试完了,你不能理解**
这个时候你去问人是恰当的,然后别人告诉你去console自己调试一下,于是你发现:![](_v_images/20191215101725942_10223.png =268x),哈???
那为什么页面上显示的不一样?**你猜**可能是`document.write()`方法会忽略某些输出.
**于是你搜索了一下**
发现有js中有三种输出方式,于是你都试了一下,发现只有`document.write()`这个在页面上显示的不完整.于是你继续搜索document.write()的使用方法,你发现![](_v_images/20191215103044402_21130.png =265x),果然像`<p>`这种标签式的,document.write()是不会打印给html页面的.
**所以这个问题才算基本解决了**
### 1.3.2. P-mission小程序后端
#### 1.3.2.1. 删掉数据后发现无法加载出页面了
- Try1
...
### 1.3.4. Hadoop云盘
#### 1.3.4.1. JAVA_HOME is not defined correctly.
**输入**
`ant -version`
**返回**
```bash
...
#### 1.3.4.2. 同一台linux的shell
**输入**
`java -version`
**返回**
![](_v_images/20200521104334775_32117.png)
#### 1.3.4.3. 提示输出目录不存在
...
![](_v_images/20200522093637364_2606.png)

#### 1.3.4.5. java.lang.NullPointerException
**点击**
`java build path`
**解决**
更换了IDE
#### 1.3.4.6. An internal error occurred during: "Map/Reduce location status updater
### 1.3.5. 各种安装问题
#### 1.3.5.1. python安装
##### 1.3.5.1.1. `ERROR: scipy-1.4.1-cp39-cp39-win_amd64.whl is not a supported wheel on this platform.`
版本不对，可能是因为少了一个`m`
...
- `pp`:只适用`pypy`
- `cp`：只适用`Cpython`
#### 1.3.5.2. MySQL 8+ No installed安装
No installed版本，也即是直接一个压缩包解压后自己玩的那种，要解决两个问题：

1. 如何跳过第一次连接服务器时候的验证，然后给它重置密码？
2. 如何使得启动的mysqld不很快崩溃？
```sh
2020-10-03T08:19:46.632797Z 0 [ERROR] [MY-010131] [Server] TCP/IP, --shared-memory, or --named-pipe should be configured on NT OS
```
3. 如何在不崩溃情况下建立在指定的端口连接？
```sh
2020-10-03T08:21:56.118759Z 0 [System] [MY-010931] [Server] D:\Software\mysql-8.0.21\bin\mysqld.exe: ready for connections. Version: '8.0.21'  socket: ''  port: 0  MySQL Community Server - GPL.
```
第一个用到`--skip-grant-tables`，重置密码也没问题；第二个经提示知道要用`named pipe`或者`shred memory`通信，这也没问题；第三个即使指定`port=3306`也是无用的，最终的结果就是无法连接。
给mysql官方提[相关bug](https://bugs.mysql.com/bug.php?id=90554)的同学如是说：
...
最后再开No installed的mysqld也只是再开另一个进程。
### 1.3.6. Socket编程
#### 1.3.6.1. 1+1!=2的bug
![](_v_images/20200607164005000_6306.png)
**原因**
是因为"EOF"背后的编码格式不一样,导致他们占用的内存大小不一样,一个是3字节,一个是6字节.
### 1.3.7. 其他
#### 1.3.7.1. Java数组的长度最大是2^31
像这样的代码编译是通不过的:
```java
...
#### 1.3.7.2. 为什么不能直接比较对象之链表结点
![](_v_images/20200924171749530_31979.png)
结点还包括结点后面的信息，递归包含。事实上这样也不对：
![](_v_images/20200925115737645_26631.png)
真的是比较地址的啊！

#### 1.3.7.3. 404却是因为文件找不到?
```sh
List<RegionOfChina> regionOfChinaList = ExcelUtil.readXls( folderPath2+"中国省市区.xlsx",RegionOfChina.class);
```
folderPath2写错了,按理说应该报文件找不到,结果最后报的是`404`.